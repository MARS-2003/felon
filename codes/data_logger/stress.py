...
def run(zip_path):
    ...
    try:
        for img_path in image_files:
            ...
            # Model expects 320x240, img is 512x512
            ...
            final_boxes = cand_boxes[keep]
            final_cls = cand_cls[keep]
            final_scores = cand_scores[keep]
            
            # --- Annotate and Save Frame ---
            for i in range(len(final_boxes)):
                box = final_boxes[i]
                # Map coordinates from 320x240 back to 512x512
                x1 = int(box[0] * w_orig / W_model)
                y1 = int(box[1] * h_orig / H_model)
                x2 = int(box[2] * w_orig / W_model)
                y2 = int(box[3] * h_orig / H_model)
                
                cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
                label = f"ID:{int(final_cls[i])} {final_scores[i]:.2f}"
                cv2.putText(img, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

            save_path = os.path.join(output_frames_dir, f"frame_{_frame_idx:06d}.jpg")
